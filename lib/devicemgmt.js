...
class dmNetTools extends dmUtils.DeviceManagement {
	...
	async listDevices() {
		...
		for (const i in devices) {
			...
			if(devices[i].native.ip || devices[i].native.mac || devices[i].native.vendor) {
				hasDetails = true;
			}
			...
			const res = {
				...
				icon: devices[i].common.icon ? devices[i].common.icon : '',
	...
	async handleSettingsDevice(id, context) {
		...
		if(result.ip !== device.native.ip || result.pingInterval !== device.native.pingInterval || result.retries !== device.native.retries) {
			const obj = {
				native: {
					ip: result.ip,
